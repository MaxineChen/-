...
class ArticleList{
	...
	ArticleList(){
		...
		try{
			  ...
			  ResultSet rs1=stmt.executeQuery("SELECT COUNT(*) as acount FROM news;");
			  if(rs1.next()) articleCount=rs1.getInt("acount");
			  list=new Article[articleCount];
			  
			  ResultSet rs2=stmt.executeQuery("SELECT * FROM news order by aid desc;");
			  int i=0;
			  while(rs2.next()) {
    ...
    <body id="list-Page-Body">
        ...
        <table>
	        <%for(int i=0;i<articleList.list.length;i++){ %>
	        	<tr>
	        		<td><input type="checkbox" name="delete" value=<%= articleList.list[i].articleId%>></td>
